...
vector<uint64_t> PAHEHelper::ntt_forward_plaintext(vector<uint64_t>& data,
  ...
  for (uint32_t logm = 1; logm <= logn; logm++) {
    for (uint32_t j = 0; j < phim; j = j + (1 << logm)) {
      for (uint32_t i = 0; i < (uint32_t)(1 << (logm - 1)); i++) {
        ...
        if (result[indexOdd][0] != 0 || result[indexOdd][1] != 0 ||
            result[indexOdd][2] != 0) {
          asmMulModQ(omegaFactor.data(), result[indexOdd].data(), omega.data());
          asmAddModQ(butterflyPlus.data(), result[indexEven].data(),
                     omegaFactor.data());
          asmSubModQ(butterflyMinus.data(), result[indexEven].data(),
                     omegaFactor.data());
...
vector<uint64_t> PAHEHelper::ntt_inverse_plaintext(vector<uint64_t>& data) {
...
vector<uint64_t> PAHEHelper::ftt_forward_plaintext(vector<uint64_t>& data) {
...
vector<uint64_t> PAHEHelper::ftt_inverse_plaintext(vector<uint64_t>& data) {
  auto ret = ntt_forward_plaintext(data, RoUInverseTablePlaintext);
...
vector<vector<uint64_t>> PAHEHelper::ftt_inverse_ciphertext(
  ...
  for (uint32_t i = 0; i < phim; i++) {
    asmMulModQ(ret[i].data(), ret[i].data(),
               scaledInverseTableCiphertext[i].data());
  }
...
void PAHEHelper::ftt_precompute_plaintext() {
  // Precomputes twiddle factor omega and FTT parameter phi for Forward
  // Transform
  uint64_t x = 1;
  ...
  // Precomputes twiddle factor omega and FTT parameter phi for Inverse
  // Transform
  x = 1;
  ...
  for (int i = 0; i < phim; i++) {
    asmMulModP(&table_scaled_inverse[i], &nInvPlaintext,
               &RoUInverseTablePlaintext[i]);
  }
...
void PAHEHelper::ftt_precompute_ciphertext() {
  ...
  for (int i = 0; i < phim; i++) {
    asmMulModQ(table_scaled_inverse[i].data(), nInvCiphertext.data(),
               RoUInverseTableCiphertext[i].data());
  }
